/*=============================================================================================================
Open Dental 
Copyright 2003-2022  Jordan Sparks, DMD.  http://www.opendental.com
...
namespace OpenDental{
    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		public void FormOpenDentalShown() {
			...
			if(!ODBuild.IsTrial() && isBackupReminderNeeded) {
				using FormBackupReminder formBackupReminder=new FormBackupReminder();
			...
			if(CultureInfo.CurrentCulture.NumberFormat.CurrencyDecimalDigits != 2 //We want our users to have their currency decimal setting set to 2.
				...
				using FormDecimalSettings formDecimalSettings=new FormDecimalSettings();
		...
		private void toolButPatient_Click() {
			...
			Patient patient2=Patients.GetPat(PatNumCur);
			if(controlChart.Visible) {
		...
		private void LayoutControls(){
			...
			LayoutManager.Move(splitContainerNoFlickerDashboard,new Rectangle(pointPosition.X,pointPosition.Y,width,height));
			...
			Point pointSplitContainer=splitContainerNoFlickerDashboard.Location;
		...
		private void menuLetter_Click(object sender,System.EventArgs e) {
			...
			if(((MenuItem)sender).Tag.GetType()==typeof(Referral)) {
				Referral referral=(Referral)((MenuItem)sender).Tag;
				using FormSheetPicker formSheetPicker=new FormSheetPicker();
				formSheetPicker.SheetType=SheetTypeEnum.ReferralLetter;
				formSheetPicker.ShowDialog();
				if(formSheetPicker.DialogResult!=DialogResult.OK){
					return;
				}
		...
		private bool toolButTxtMsg_Click(long patNum,string startingText="") {
			...
			formTxtMsgEdit2.TxtMsgOk=patient.TxtMsgOk;
		...
		public override void ProcessSignalODs(List<Signalod> listSignalods) {
			...
			if(controlAccount.Visible && signalodPP!=null && signalodPP.FKey==controlAccount.PatCur.PatNum) {
				RefreshCurrentModule();
			}
	...
	private void TaskGoTo(TaskObjectType taskObjectType,long keyNum){
		...
		private void menuItemUserSettings_Click(object sender,EventArgs e) {
			using FormUserSetting formUserSetting=new OpenDental.FormUserSetting();
		...
		private void menuItemZipCodes_Click(object sender, System.EventArgs e) {
			...
			//	return;
			//}
			using FormZipCodes formZipCodes=new FormZipCodes();
			formZipCodes.ShowDialog();
		...
		private void menuItemCreateAtoZFolders_Click(object sender,EventArgs e) {
			...
			using FormAtoZFoldersCreate formAtoZFoldersCreate=new FormAtoZFoldersCreate();
			formAtoZFoldersCreate.ShowDialog();
			//Security log entries are made from within the form.
		}
